...
import { renderTaxCompliance } from "./views/tax-compliance";
import { renderVatReturns } from "./views/vat-returns";
import { renderEvidenceLedger } from "./views/evidence-ledger";
import { renderBankReconciliation } from "./views/bank-reconciliation";
...
import { renderTeamChatView, type TeamAgentInfo, type TeamChatMessage } from "./views/team-chat-view";
 ...
 */
function isTeamSessionKey(sessionKey: string): boolean {
 ...
 */
function parseTeamId(sessionKey: string): string | null {
...
/**
 * Get team members from agents list for team chat view.
 */
function getTeamMembersForChat(agentsList: AgentsListResult | null): TeamAgentInfo[] {
  if (!agentsList?.agents) return [];
  return agentsList.agents.map((agent) => ({
    id: agent.id,
    name: agent.identity?.name ?? agent.name ?? agent.id,
  ...
  return html`
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
      ...
      ? renderExecutiveDashboard({
        connected: state.connected,
        jurisdiction: (state.executiveJurisdiction ?? "MT") as JurisdictionCode,
        kpis: state.executiveKpis,
        engagements: state.executiveEngagements,
        onJurisdictionChange: async (code) => {
          state.executiveJurisdiction = code;
          state.executiveLoading = true;
          try {
            const data = await loadExecutiveDashboard(state.client, code);
            state.executiveKpis = data.kpis;
            state.executiveEngagements = data.engagements;
          } catch (err) {
            state.executiveError = String(err);
          } finally {
        ...
        onRefresh: async () => {
          ...
          try {
            const data = await loadExecutiveDashboard(
              state.client,
              (state.executiveJurisdiction ?? "MT") as JurisdictionCode
            );
            state.executiveKpis = data.kpis;
            state.executiveEngagements = data.engagements;
          } catch (err) {
      ...
      ? renderTaxCompliance({
        connected: state.connected,
        periods: state.taxPeriods,
        evidenceStats: state.taxEvidenceStats,
        pendingApprovals: state.taxPendingApprovals,
        onRefresh: () => refreshTaxCompliance(state),
      })
      : nothing
    }
      ...
      ? renderAmlDashboard({
        connected: state.connected,
        loading: false,
        cddStats: { total: 0, pending: 0, overdue: 0, byRisk: { low: 0, medium: 0, high: 0 } },
        strQueue: [],
        alerts: [],
        onRefresh: () => { },
      })
      : nothing
        ...
        ${state.tab === "chat"
      ? (isTeamSessionKey(state.sessionKey)
        ? renderTeamChatView({
          connected: state.connected,
          loading: state.chatLoading,
          error: state.lastError ?? null,
          teamId: parseTeamId(state.sessionKey) ?? "unknown",
          teamName: "Prisma CPA Staff Team", // TODO: Get from config
          members: getTeamMembersForChat(state.agentsList),
          messages: [], // TODO: Wire to team messages storage
          inputValue: state.chatMessage,
          onInputChange: (value) => (state.chatMessage = value),
          onSendMessage: () => {
            // TODO: Implement team message sending
            console.log("Team message:", state.chatMessage);
            state.chatMessage = "";
          },
          onBack: () => {
            state.tab = "agents";
